						         ...
						         <?php
									...
									//$tiene_grabaciones = (!isset($evaluacion['tiene_grabaciones_'.$i])) ? $evaluacion['tiene_grabaciones_'.$i] : 'asdf' ;
							      	...
							      	for ($i=0; $i < $evaluaciones[0]['cant_campanias']; $i++) { 
							      		...
							      		{		      			
								        	echo '<a href="'.base_url().'Evaluacion/AgregarEvaluacion/?idEAC='.$evaluacion['cod_usuario'].'&idCamp='.$evaluacion[('id_camp_'.$i)].'&codCamp='.$evaluacion[('cod_camp_'.$i)].'&idUsuResp='.$id_usuario.'" data-toggle="tooltip" data-placement="top" title="click para gestionar" class="badge badge-pill ';
